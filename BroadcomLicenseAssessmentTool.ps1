...
function Ensure-ExecutionPolicy {
    ...
    Write-Log ("Prereq OK - Execution policy: {0}" -f $summary) 'OK' 'Green'

    $processRow = $policies | Where-Object { $_.Scope -eq 'Process' } | Select-Object -First 1
    $processPolicy = if ($processRow) { [string]$processRow.ExecutionPolicy } else { '' }
